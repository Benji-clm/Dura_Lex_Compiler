...
NodePtr
Parse (const std::string &compile_source_path)
{
...
void
PrettyPrint (const NodePtr &root, const std::string &compile_output_path)
{
...
void
Compile (const NodePtr &root, const std::string &compile_output_path)
{
  ...
  std::string dest_reg = "a0";
  std::ofstream output (compile_output_path, std::ios::trunc);
  root->EmitRISC (output, ctx, dest_reg);

  ctx.emitLabels (output);
